...
export default {
  ...
  computed: {
    ...mapState({
      allTags: state => state.hexo.tags.map(cat => cat.name)
    }),
    availableTags () {
      const at = []
      at.push.apply(at, this.allTags)
    ...
    postTags: {
      get () {
        return this.$store.state.hexo.post.tags || []
      },
      set (v) {
        this.$store.commit('hexo/SET_POST_TAGS', v)
      }
    }
  },
  methods: {
